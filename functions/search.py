...
def search_google(query: str, dry_run: bool = True, **kwargs: dict) -> list[dict]:
    ...
    Examples:
        >>> search_google(query='What is young sheldon?', dry_run=False)
        [
        ...
        >>> search_google(query='How to use pandas in python?', dry_run=False)
        [
        ...
        >>> search_google(query='Best iPad games?', dry_run=False)
        [
        ...
        >>> search_google(query='Best iPad games?', dry_run=True)
        [
            {'state': 'success'}
        ]
    """
